...
(function () {
    ...
    window.jQuery('form.variations_form')
      .on('found_variation', function (e, v) {
        if (v) {
          ...
          var m = modelKey(v.attributes && (v.attributes.attribute_pa_model || v.attributes.attribute_model || ''));
          if (m && m !== caseModel) {
   ...
   *
   * KEEP IN SYNC with the identical helper in combos.js.
   */
  function modelKey(raw) {
    if (!raw) return '';
    var map = (window.GALADO_COMBOS || {}).models || {};
